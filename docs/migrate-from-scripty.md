...
5. **Strands can't cross an external call boundary.** A `Strand` is a memory pointer — internal calls share one memory space, so compose lazily and pass strands freely through them, then materialize with `.toString()` exactly once where the result leaves the frame (an external return, another contract, storage, an event). That single materialization is where the gas goes, which is the point of staying lazy until then. (scripty returned materialized `bytes`, so this footgun didn't exist there.)

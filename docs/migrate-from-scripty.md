...
5. **Strands can't be returned, stored, or passed to other contracts.** A `Strand` is a memory pointer — compose lazily and pass strands around freely *within* the call, then materialize with `.toString()` exactly once, at the boundary. That single materialization is where the gas goes, which is the point of staying lazy until then. (scripty returned materialized `bytes`, so this footgun didn't exist there.)

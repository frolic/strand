...
2. **No render-time base64.** scripty's `HTMLTagType.script` base64-encodes raw content inside `getHTMLURLSafe` (its own docs warn this risks gas-out). Strand doesn't encode bytecode content at all — store scripts base64-encoded (the EthFS convention), which is also what scripty recommends. When you do need encoding at render time, prefer `encodeURI()` over base64: percent-encoding is byte-local, so it passes already-safe bytes (like base64 payloads) through at 1× growth and zero work, while base64 re-encodes everything at a fixed 4/3 and requires materializing the full string first.
...
5. **A `Strand` is a memory pointer, so it doesn't survive across external calls — materialize with `.toString()` first.** Internal calls share one memory space, so compose lazily and pass strands freely through them; materialize exactly once where the result leaves the frame (an external return, another contract, storage, an event). That single materialization is where the gas goes, which is the point of staying lazy until then. (scripty returned materialized `bytes`, so this footgun didn't exist there.) If a strand genuinely needs to cross, encode the recipe instead of the pointer — `abi.encode(_unwrap(strand))`, decoded with `_wrap(abi.decode(recipe, (_Strand)))` on the other side; see the README design notes.

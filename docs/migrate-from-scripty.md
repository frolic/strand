...
| `contractAddress` + `ScriptyStorageV2` | `sstore2(pointer)` |
...
5. **A `Strand` is a memory pointer, so it can't be used across contract boundaries.** Internal calls share one memory space, so strands flow freely through them; to cross a boundary, pass the *recipe* — `serialize(strand)`, rebuilt with `deserialize(recipe)` on the other side (see the README design notes) — rather than a materialized string, which would pay the full ABI round-trip on the output size. Materialize with `.toString()` exactly once, at the final consumer; that's where the gas goes, which is the point of staying lazy until then. (scripty returned materialized `bytes`, so neither the footgun nor the savings existed there.)

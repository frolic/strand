...
- **3 fixed:** `_codeSlice` reverts on `start > end` (folded into the existing bounds check). Follow-up: the three-arg `bytecode` now also rejects `end <= start` at construction — no `extcodesize` needed for that check — so only size-dependent validation is deferred to render (`test_bytecodeConstructorRejectsInvalidRange`).

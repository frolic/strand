...
- **A `Strand` is a memory pointer.** Internal calls all share one memory space, so compose lazily and pass strands freely through them — then materialize with `toString()` exactly once when the result must cross an *external* call boundary. Never use a `Strand` as a public/external parameter or return type, and never store one: ABI-encoding it sends the raw pointer, and the receiving frame dereferences its own unrelated memory.

...
- **A `Strand` is a memory pointer.** It lives and dies inside the call frame that built it. Compose lazily and pass strands around freely within that frame, then materialize with `toString()` exactly once, at the boundary — never use one as a public/external parameter or return type, and never store one. ABI-encoding a `Strand` across a call boundary sends the raw pointer, and the receiving frame dereferences its own unrelated memory.

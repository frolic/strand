...
- **A `Strand` is a memory pointer, so it can't travel outside the contract it's used within — materialize with `toString()` first.** Internal calls share one memory space, so compose lazily and pass strands freely through them. Never use a `Strand` as a public/external parameter or return type, and never store one: ABI-encoding it sends the raw pointer, and the receiving frame dereferences its own unrelated memory. You *can* work around this by encoding/decoding the underlying struct, which serializes the recipe rather than the pointer — inline literals travel by value, bytecode parts as small `(pointer, start, end)` references that stay valid everywhere because contract code is global state:

  ```solidity
  // sender: serialize the recipe (stays small no matter how big the referenced data is)
  bytes memory recipe = abi.encode(_unwrap(strand));

  // receiver: decode into this frame's memory and keep composing lazily
  Strand strand = _wrap(abi.decode(recipe, (_Strand)));
  ```

  The receiver inherits trust in the pointers — only decode recipes from senders you trust.
